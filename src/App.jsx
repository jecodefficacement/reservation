...
// ─────────────────────────────────────────
const LS_KEY = "jecode_reservations_v1";
...
async function sendToGoogleSheet(entry) {
  if (!GOOGLE_SHEET_URL || GOOGLE_SHEET_URL.startsWith("REMPLACE")) return;
  try {
    ...
    console.warn("Google Sheet non configuré :", e.message);
  }
}
...
async function sendEmailNotification(entry) {
  ...
  try {
    ...
    console.warn("EmailJS non configuré :", e.message);
  }
}
...
function Card({ children, style = {} }) {
  return (
    <div style={{
      ...
      boxShadow: "0 8px 32px rgba(70,0,140,0.12)",
      ...style
    }}>
      {children}
    </div>
...
function Field({ label, error, children }) {
  return (
    <div style={{ marginBottom: 18 }}>
      ...
      {children}
      {error && (
        <p style={{ color: C.rose, fontSize: "0.78rem", marginTop: 5, display: "flex", alignItems: "center", gap: 4 }}>
          <span>⚠</span> {error}
        </p>
      )}
    </div>
...
function Badge({ children, bg = "rgba(255,255,255,0.18)", color = C.white, style = {} }) {
  return (
    <span style={{
      ...
      fontSize: "0.82rem", fontWeight: 600, ...style
    }}>
      {children}
    </span>
  );
...
function Logo({ dark = false }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{
        background: dark ? C.violet : C.white,
        borderRadius: 9, padding: "4px 10px",
        fontFamily: "'JetBrains Mono', monospace",
        fontWeight: 700, color: dark ? C.white : C.violet,
        fontSize: "1rem", letterSpacing: -0.5,
      }}>
        &lt;/&gt;
      </div>
      <span style={{ color: dark ? C.violet : C.white, fontWeight: 900, fontSize: "1.2rem", letterSpacing: -0.5 }}>
        JeCode
      </span>
    </div>
  );
...
function PublicPage({ onAdmin }) {
  ...
  const submit = async () => {
    ...
    lsSave([entry, ...existing]);

    // Envois externes (en parallèle, non bloquants)
  ...
  if (done) return (
    <div style={{ background: gradBg, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem" }}>
      <Card style={{ maxWidth: 480, width: "100%", textAlign: "center" }}>
        ...
        <h2 style={{ color: C.violet, fontWeight: 900, fontSize: "1.7rem", marginBottom: 10 }}>
          Place réservée !
        </h2>
        <p style={{ color: C.textSoft, lineHeight: 1.7, marginBottom: 20 }}>
          Merci <strong style={{ color: C.rose }}>{form.nom}</strong> !<br />
          Nous te contacterons au <strong>{form.telephone}</strong> pour confirmer ta place et te donner tous les détails.
        </p>
        <div style={{
          background: "#F8F4FF", borderLeft: `4px solid ${C.violet}`,
          borderRadius: 12, padding: "1rem", textAlign: "left", marginBottom: 20, fontSize: "0.88rem", color: C.textSoft
        }}>
          💡 <strong>Rappel :</strong> le paiement de <strong>500 000 GNF</strong> se fait uniquement en présentiel, le jour de ton inscription à Kountia OAS.
        </div>
        <a
          href={`https://wa.me/224624144006`}
          target="_blank" rel="noreferrer"
          style={{
            display: "block", background: C.green, color: C.white,
            borderRadius: 12, padding: "0.85rem", fontWeight: 700,
            textDecoration: "none", fontSize: "0.95rem", marginBottom: 12
          }}
        >
          💬 Une question ? WhatsApp : +224 624 144 006
        </a>
        <button
          onClick={() => { setDone(false); setForm({ nom:"",telephone:"",email:"",niveau:"",question:"" }); }}
  ...
  return (
    <div style={{ background: gradBg, minHeight: "100vh" }}>
      ...
      <div style={{ textAlign: "center", padding: "0.5rem 1.5rem 1.5rem" }}>
        <Badge style={{ marginBottom: 14, letterSpacing: 2, fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase" }}>
        ...
        <h1 style={{ color: C.white, fontSize: "clamp(1.9rem, 5vw, 3rem)", fontWeight: 900, lineHeight: 1.1, marginBottom: 10 }}>
          Maîtrise le{" "}
          <span style={{ background: C.yellow, color: C.violet, borderRadius: 8, padding: "0 10px" }}>
            CODE
          </span>{" "}
          ces vacances
        </h1>
      ...
      <div style={{ display: "flex", justifyContent: "center", padding: "0 1rem 3rem" }}>
        <Card style={{ maxWidth: 520, width: "100%" }}>
          <h2 style={{ color: C.violet, fontWeight: 900, fontSize: "1.25rem", marginBottom: 22 }}>
            Formulaire de réservation
          </h2>

          <Field label="Nom complet *" error={errors.nom}>
            <input value={form.nom} onChange={e => set("nom", e.target.value)}
              placeholder="Ex : Mamadou Diallo" style={inputSx(!!errors.nom)} />
          </Field>

          <Field label="Numéro de téléphone *" error={errors.telephone}>
            <input value={form.telephone} onChange={e => set("telephone", e.target.value)}
              placeholder="Ex : +224 6XX XXX XXX" style={inputSx(!!errors.telephone)} />
          </Field>

          <Field label="Adresse email (optionnel)">
            <input value={form.email} onChange={e => set("email", e.target.value)}
              placeholder="Ex : mamadou@email.com" style={inputSx()} />
          </Field>
          ...
          <Field label="Question ou demande particulière (optionnel)">
            <textarea value={form.question} onChange={e => set("question", e.target.value)}
              placeholder="Tu as une question ? Quelque chose à éclaircir avant de t'inscrire ?"
          ...
          <button
            ...
            style={{
              width: "100%", background: loading ? "#ccc" : gradBtn,
              color: C.white, border: "none", borderRadius: 14,
              padding: "1rem", fontSize: "1.05rem", fontWeight: 900,
  ...
  return (
    <div style={{ background: gradBg, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <Card style={{ maxWidth: 360, width: "90%", textAlign: "center" }}>
        ...
        <input
          type="password" value={pwd} placeholder="Mot de passe"
        ...
        <button onClick={login} style={{ width: "100%", background: gradBtn, color: C.white, border: "none", borderRadius: 12, padding: "0.85rem", fontWeight: 800, cursor: "pointer", marginBottom: 10 }}>
          Accéder →
        </button>
...
function AdminDashboard({ onLogout }) {
  ...
  useEffect(() => { setList(lsLoad()); }, []);

  const save = (updated) => { lsSave(updated); setList(updated); };
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "jecode_reservations.csv"; a.click();
  };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div style={{ background: gradBg, padding: "1rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={exportCSV} style={{ background: "rgba(255,255,255,0.2)", border: "1px solid rgba(255,255,255,0.3)", color: C.white, borderRadius: 20, padding: "0.4rem 1rem", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600 }}>
            ⬇ Export CSV
          </button>
          <button onClick={onLogout} style={{ background: "rgba(255,255,255,0.2)", border: "1px solid rgba(255,255,255,0.3)", color: C.white, borderRadius: 20, padding: "0.4rem 1rem", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600 }}>
            Déconnexion
          </button>
        </div>
      </div>
            ...
            {filtered.map(r => {
              ...
              return (
                <div key={r.id} onClick={() => setSelected(isSel ? null : r)}
                  style={{
                    background: isSel ? "#EDE9F7" : C.white,
                    border: `2px solid ${isSel ? C.violet : "transparent"}`,
                    borderRadius: 14, padding: "0.9rem 1.1rem", cursor: "pointer",
                ...
                >
                  <div>
                    ...
                    <div style={{ color: C.textSoft, fontSize: "0.82rem", marginTop: 2 }}>
                      {r.telephone}{r.email ? ` · ${r.email}` : ""}
                    </div>
          ...
          {selected && (
            <div style={{ width: 290, background: C.white, borderRadius: 18, padding: "1.4rem", boxShadow: "0 4px 20px rgba(70,0,140,0.1)", position: "sticky", top: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
                ...
                <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", cursor: "pointer", color: C.muted, fontSize: "1.1rem", lineHeight: 1 }}>✕</button>
              </div>
              ...
              {selected.question && (
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ background: "#F8F4FF", borderRadius: 10, padding: "0.7rem", fontSize: "0.84rem", color: C.textSoft, lineHeight: 1.5 }}>
                    {selected.question}
                  </div>
                </div>
              )}
                  ...
                  {["En attente", "Confirmé", "Annulé"].map(s => {
                    ...
                    return (
                      <button key={s} onClick={() => setStatus(selected.id, s)}
                        style={{
                          flex: 1, padding: "0.4rem 0.2rem", borderRadius: 8, border: "none", cursor: "pointer",
                          background: active ? sm2.color : sm2.bg,
                          color: active ? C.white : sm2.color,
